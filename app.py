...
app.config["SQLALCHEMY_DATABASE_URI"] = (
    "sqlite:///" + os.path.join(basedir, "mdss.db")
)
...
# ══════════════════════════════════════════════
# DATABASE AND LOGIN CONFIGURATION
# ══════════════════════════════════════════════
...
def compute_shap_values(
    ...
    family_history = clinical_features[
        "family_history"
    ]
    ...
    prior_biopsy = clinical_features[
        "prior_biopsy"
    ]
...
def compute_risk_level(
    ...
    if combined_risk < 0.30:
        ...
        recommendation = (
            "Routine annual mammographic "
            "screening is recommended. "
            "No immediate clinical intervention "
            "required. Advise patient to maintain "
        ...
        recommendation = (
            ...
            "Consider referral to a breast specialist "
        ...
        recommendation = (
            "Immediate referral for core needle "
            "biopsy is strongly recommended. "
            "Urgent specialist review by an oncologist "
            "or breast surgeon is advised."
        )
...
def predict():
    ...
    # ── Step 3: Use first image as primary image ──

    image_path = image_paths[0]

    # ── Step 4: Get clinical information ──
    ...
    try:
        ...
        return (
            "Invalid clinical information provided.",
            400
        )

    # ── Step 5: Preprocess image ──
    # Correct indentation is important here.
    ...
    # ── Step 8: Calculate SHAP-style values ──
    ...
    (
        ...
        risk_score,
        clinical_features["birads"]
    )

    # ── Step 10: Prepare display values ──
    ...
    family_history_display = (
        "Yes"
        if clinical_features["family_history"] == 1
        else "No"
    )

    menopause_display = (
        "Post-menopausal"
        if clinical_features["menopause"] == 1
        else "Pre-menopausal"
    )

    prior_biopsy_display = (
        "Yes"
        if clinical_features["prior_biopsy"] == 1
        else "No"
    )
    ...
    density_display = density_map.get(
        clinical_features["density"],
        "B"
    )
    ...
    # ── Step 11: Save result to database ──
    ...
    try:
        ...
        new_result = AnalysisResult(
            ...
            density=clinical_features[
                "density"
            ],

            menopause=clinical_features[
                "menopause"
            ],
...
def dashboard():
    ...
    try:
        ...
        intermediate_risk = sum(
            1
            for result in all_results
            if result.risk_level == "INTERMEDIATE"
        )
...
def download_report():
    ...
    density = request.form.get(
        "density",
        ""
    )

    menopause = request.form.get(
        "menopause",
        ""
    )

    prior_biopsy = request.form.get(
        "prior_biopsy",
        ""
    )
    ...
    shap_names = request.form.getlist(
        "shap_names[]"
    )

    shap_values = request.form.getlist(
        "shap_values[]"
    )
    ...
    pdf.cell(
        0,
        10,
        "MDSS - Breast Cancer Detection Report",
        ln=True
    )
    ...
    pdf.multi_cell(
        ...
        5,
        (
            "DISCLAIMER: This AI-generated report is "
            "intended to support, not replace, clinical "
            "judgement. All findings should be reviewed "
    ...
    pdf.cell(
        ...
        6,
        (
            "MDSS - Hybrid AI Model for Breast Cancer "
            "Detection | AAUA | Subgroup 3 | Dr. Ogbeide"
        )
    )
...
def patient_history(
    ...
    if (
        ...
        and current_user.id != patient_id
    ):
